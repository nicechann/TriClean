...
struct StorageView: View {
    ...
    private func runScan(for url: URL, minSizeMB: Double, trigger: ScanTrigger) {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            let results = scanStructuredItems(of: url, minSizeMB: minSizeMB)
            ...
            DispatchQueue.main.async {
                ...
                if results.isEmpty {
                    self.scanMessage = "'\(url.lastPathComponent)' 폴더에 조건에 맞는 하위 항목이 없습니다."
                } else {
                    self.scanMessage = "'\(url.lastPathComponent)' 폴더 분석이 완료되었습니다. (폴더 \(folderCount)개 / 파일 \(fileCount)개)"
                }
}
        }
    }
...
//    let isDetailScanning: Bool
//
//    private var categories: [DiskUsageCategory] {
//        let total = max(Int64(1), info.totalBytes)
//        let used = max(Int64(0), info.usedBytes)
//
//        let home = min(max(homeBytes ?? 0, 0), used)
//        let apps = min(max(appsBytes ?? 0, 0), max(used - home, 0))
//        let otherUsed = max(used - home - apps, 0)
//        let free = max(min(info.freeBytes, total), 0)
//
//        var result: [DiskUsageCategory] = []
//
//        if home > 0 {
//            result.append(.init(name: "Home Folder", bytes: home, color: Color(red: 0.98, green: 0.46, blue: 0.33)))
//        }
//        if apps > 0 {
//            result.append(.init(name: "Applications", bytes: apps, color: Color(red: 0.99, green: 0.77, blue: 0.30)))
//        }
//
//        // Home/Apps 분석 전(또는 0)이라도, 전체 사용량은 막대에 보여야 해서 Other Used는 항상 넣습니다.
//        if otherUsed > 0 {
//            result.append(.init(name: "Other Used", bytes: otherUsed, color: Color(red: 0.35, green: 0.70, blue: 0.90)))
//        }
//        if free > 0 {
//            result.append(.init(name: "Free Space", bytes: free, color: Color.gray.opacity(0.60)))
//        }
//        return result
//    }
//
...
//            loadDiskInfo()
//
//            // ✅ App Store 제출용(보수적): 사용자가 선택한 폴더(보안 스코프)만 다룹니다.
//            if selectedFolderURL == nil,
//               let restored = SecurityScopedBookmarkStore.shared.resolveURL(for: .storageRoot) {
...
//        isHomeScanning = true
//        DispatchQueue.global(qos: .userInitiated).async {
//            let size = folderSizeBytes(at: home)
//            DispatchQueue.main.async {
//                self.homeFolderBytes = size
//                self.isHomeScanning = false
//            }
...
//
//    private func scanApplicationsFolder() {
//        let systemAppsURL = URL(fileURLWithPath: "/Applications", isDirectory: true)
//        let userAppsURL = FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent("Applications", isDirectory: true)
//
//        isAppsScanning = true
//        DispatchQueue.global(qos: .userInitiated).async {
//            let fm = FileManager.default
//            var total: Int64 = 0
//
//            if fm.fileExists(atPath: systemAppsURL.path) {
//                total += folderSizeBytes(at: systemAppsURL)
//            }
//            if fm.fileExists(atPath: userAppsURL.path) {
//                total += folderSizeBytes(at: userAppsURL)
//            }
//
//            DispatchQueue.main.async {
//                self.appsFolderBytes = total
...
//
//            do {
//                try SecurityScopedBookmarkStore.shared.save(url: url, for: .storageRoot)
//            } catch {
//                // 북마크 저장 실패해도(권한/파일시스템 이슈) 즉시 스캔은 시도할 수 있습니다.
...
//            scanMessage = "폴더 접근 권한이 없습니다. 다시 폴더를 선택해 주세요."
//            return
//        }
//        
//        DispatchQueue.global(qos: .userInitiated).async {
...
//
//    private func moveToTrash(_ url: URL) {
//        // ✅ 보수적(App Store): 선택 폴더/항목의 보안 스코프 권한이 있을 때만 삭제(휴지통 이동)
//        let token =
//            SecurityScopedAccessToken(url: url)
//            ?? (selectedFolderURL.flatMap { SecurityScopedAccessToken(url: $0) })
//
//        do {
//            try FileManager.default.trashItem(at: url, resultingItemURL: nil)
//        } catch {
