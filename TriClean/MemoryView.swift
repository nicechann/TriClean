//
//  MemoryView.swift         ← 반드시 이 파일명으로 저장
//  TriClean
//
...
struct MemoryDonutView: View {
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            ZStack {
                Circle().stroke(ringBackgroundColor, lineWidth: lineWidth)
    ...
    private func donutSegments(size: CGFloat, lineWidth: CGFloat) -> some View {
        ...
        let availableRatio  = max(0.0, 1.0 - usedRatio)
        ...
        return ZStack {
            ...
            Circle()
                ...
                .rotationEffect(startAngle)
            Circle()
                .trim(from: CGFloat(usedRatio), to: CGFloat(min(1.0, usedRatio + availableRatio)))
